...
public class EnemyGroundBasicMovement : EnemyMovement
{
	...
	{
		if (!(Physics2D.OverlapCircle(new Vector2(cliffCheck.position.x, cliffCheck.position.y), 0.03f,
									  1 << LayerMask.NameToLayer("Ground"))
		   || Physics2D.OverlapCircle(new Vector2(cliffCheck.position.x, cliffCheck.position.y), 0.03f,
	...
	{
		if (Physics2D.OverlapCircle(new Vector2(wallCheck.position.x, wallCheck.position.y), 0.03f,
									1 << LayerMask.NameToLayer("Ground")))
		{
			willGoRight = !willGoRight;
		}
