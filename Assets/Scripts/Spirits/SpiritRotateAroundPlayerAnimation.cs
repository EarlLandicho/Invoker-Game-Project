...
public class SpiritRotateAroundPlayerAnimation : MonoBehaviour
{
	...
	private void RotateSpiritAroundPlayer()
	{
		...
		spirit1.transform.position = transform.position +
									 new Vector3(Mathf.Sin(spiritRotationAngle), Mathf.Sin(spiritRotationAngle),
												 Mathf.Cos(spiritRotationAngle)) * spiritRotationRadius;
		spirit2.transform.position = transform.position +
									 new Vector3(Mathf.Sin(spiritRotationAngle + oneThirdOfACircle),
												 Mathf.Cos(spiritRotationAngle + oneThirdOfACircle),
												 Mathf.Cos(spiritRotationAngle + oneThirdOfACircle)) *
									 spiritRotationRadius;
		spirit3.transform.position = transform.position +
									 new Vector3(Mathf.Sin(spiritRotationAngle + twoThirdsOfACircle),
												 Mathf.Cos(spiritRotationAngle),
												 Mathf.Cos(spiritRotationAngle + twoThirdsOfACircle)) *
									 spiritRotationRadius;
	}
}