...
public class TestingZLegacy : MonoBehaviour
{
	...
	{
		/**/
		"updateValue3Example", "loopTestClamp", "loopTestPingPong",
		"moveOnACurveExample", "customTweenExample", "moveExample",
		"rotateExample", "scaleExample", "updateValueExample",
		"delayedCallExample", "alphaExample", "moveLocalExample",
		"rotateAroundExample", "colorExample"
	};
...
//		alphaExample();
	...
	private void cycleThroughExamples()
	{
		if (exampleIter == 0)
		{
			...
			{
				descrTimeScaleChangeId = LeanTween.value(gameObject, 0.01f, 10.0f, 3f).setOnUpdate(onUpdate: val =>
																											 {
	...
	public void updateValueExample()
	{
		...
		LeanTween
		   .value(gameObject, (Action<float, object>) updateValueExampleCallback, ltLogo.transform.eulerAngles.y, 270f,
				  1f).setEase(LeanTweenType.easeOutElastic).setOnUpdateParam(pass)
		   .setUseEstimatedTime(useEstimatedTime);
	}
