...
namespace DentedPixel.LTExamples
{
	public class TestingUnitTests : MonoBehaviour
	{
		...
		private void Start()
		{
			...
			LeanTween.scaleX(cube4, 2f, 0f).setOnComplete(onComplete: () =>
																	  {
																		  LeanTest.expect(cube4.transform.localScale.x == 2f,
																						  "TWEENED WITH ZERO TIME");
																	  });
			...
			LeanTween.delayedCall(cubes[0], 0.2f, callback: () =>
															{
																for (int i = 0; i < cubes.Length; i++)
																{
																	if (i % 3 == 0)
																		...
																		descr.setOnComplete(onComplete: () =>
																										{
																											...
																											{
																												LeanTest.expect(true,
																																"CANCELS DO NOT EFFECT FINISHING");
																											}
																										});
																	}
																}
			...
			LeanTween.move(cubeToTrans, cubeDest.transform, 1.2f).setEase(LeanTweenType.easeOutQuad)
					 .setOnComplete(onComplete: () =>
												{
													LeanTest.expect(cubeToTrans.transform.position == cubeDestEnd,
			...
			LeanTween.moveX(cubeDestroy, 200f, 0.1f).setDestroyOnComplete(true)
					 .setOnComplete(onComplete: () => { LeanTest.expect(true, "TWO DESTROY ON COMPLETE'S SUCCEED"); });
			...
			LeanTween
			   .moveSpline(cubeSpline,
						   ...
						   }, 0.1f).setOnComplete(onComplete: () =>
															  {
																  LeanTest
																	 .expect(Vector3.Distance(new Vector3(1f, 0f, 1f), cubeSpline.transform.position) < 0.01f,
			...
			LeanTween.delayedCall(gameObject, 0.2f, callback: () =>
															  {
																  LTDescr d = LeanTween.descr(jumpTimeId);
																  float beforeX = jumpCube.transform.position.x;
																  d.setTime(0.5f);
																  LeanTween.delayedCall(0.0f, callback: () => { }).setOnStart(onStart: () =>
																															  {
																																  float
																																	  diffAmt =
																																  ...
																																  LeanTest
																																	 .expect(Mathf.Abs(jumpCube.transform.position.x - beforeX) < diffAmt,
																																		  "CHANGING TIME DOESN'T JUMP AHEAD",
																																		  "Difference:" +
																																		  Mathf.Abs(jumpCube.transform
																																				 .position.x -
																																			  beforeX) +
																																		  " beforeX:" + beforeX + " now:" +
			...
			LeanTween.moveX(zeroCube, 10f, 0f).setOnComplete(onComplete: () =>
																		 {
																			 LeanTest.expect(zeroCube.transform.position.x == 10f,
			...
			LeanTween.rotateAround(cubeRotateB, Vector3.forward, 360f, 0.3f).setPoint(new Vector3(5f, 3f, 2f))
					 .setOnComplete(onComplete: () =>
												{
													LeanTest
													   .expect(cubeRotateB.transform.position.ToString() == (new Vector3(200f, 10f, 8f)).ToString(),
															   "ROTATE AROUND 360",
															   "expected rotate pos:" + (new Vector3(200f, 10f, 8f)) +
															   " returned:" + cubeRotateB.transform.position);
												});

			// Alpha, onUpdate with passing value, onComplete value
			LeanTween.alpha(cubeAlpha1, 0.5f, 0.1f).setOnUpdate(onUpdate: val => { LeanTest.expect(val != 0f, "ON UPDATE VAL"); })
					 .setOnCompleteParam("Hi!").setOnComplete(onComplete: completeObj =>
																		  {
																			  LeanTest.expect(((string) completeObj) == "Hi!",
																							  "ONCOMPLETE OBJECT");
																			  LeanTest
																				 .expect(cubeAlpha1.GetComponent<Renderer>().material.color.a == 0.5f,
																						 "ALPHA");
																		  });
			// Color
			float onStartTime = -1f;
			LeanTween.color(cubeAlpha2, Color.cyan, 0.3f).setOnComplete(onComplete: () =>
																					{
																						LeanTest
																						   .expect(cubeAlpha2.GetComponent<Renderer>().material.color == Color.cyan,
																								   "COLOR");
																						LeanTest
																						   .expect(onStartTime >= 0f && onStartTime < Time.time,
																								   "ON START",
																								   "onStartTime:" + onStartTime +
																								   " time:" + Time.time);
																					}).setOnStart(onStart: () => { onStartTime = Time.time; });
			// moveLocalY (make sure uses y values)
			Vector3 beforePos = cubeAlpha1.transform.position;
			LeanTween.moveY(cubeAlpha1, 3f, 0.2f).setOnComplete(onComplete: () =>
																			{
																				LeanTest
																				   .expect(cubeAlpha1.transform.position.x == beforePos.x && cubeAlpha1.transform.position.z == beforePos.z,
																						   "MOVE Y");
																			});
			Vector3 beforePos2 = cubeAlpha2.transform.localPosition;
			LeanTween.moveLocalZ(cubeAlpha2, 12f, 0.2f).setOnComplete(onComplete: () =>
																				  {
																					  LeanTest
																						 .expect(cubeAlpha2.transform.localPosition.x == beforePos2.x && cubeAlpha2.transform.localPosition.y == beforePos2.y,
																								 "MOVE LOCAL Z",
																								 "ax:" +
																								 cubeAlpha2.transform
																										   .localPosition.x + " bx:" +
																								 beforePos.x + " ay:" +
																								 cubeAlpha2.transform
																										   .localPosition.y + " by:" +
			...
			AudioClip audioClip =
				LeanAudio.createAudio(new AnimationCurve(new Keyframe(0f, 1f, 0f, -1f), new Keyframe(1f, 0f, -1f, 0f)),
									  new AnimationCurve(new Keyframe(0f, 0.001f, 0f, 0f),
														 new Keyframe(1f, 0.001f, 0f, 0f)), LeanAudio.options());
			LeanTween.delayedSound(gameObject, audioClip, new Vector3(0f, 0f, 0f), 0.1f).setDelay(0.2f)
					 .setOnComplete(onComplete: () => { LeanTest.expect(Time.time > 0, "DELAYED SOUND"); });
			...
			for (int j = 0; j < 2; j++)
			{
				...
				for (int i = (int) LeanTweenType.notUsed; i < totalTweenTypeLength; i++)
				{
					...
					LTDescr descr = LeanTween.moveLocalX(cube, 5, 0.1f).setOnComplete(onComplete: obj =>
																								  {
			...
			LeanTween.value(gameObject, new Vector2(0, 0), new Vector2(256, 96), 0.1f)
					 .setOnUpdate(onUpdate: (Vector2 value) => { value2UpdateCalled = true; });
			LeanTween.delayedCall(0.2f, callback: () => { LeanTest.expect(value2UpdateCalled, "VALUE2 UPDATE"); });
		...
		private IEnumerator timeBasedTesting()
		{
			...
			LeanTween.moveX(cubeNormal, 12f, 1.5f).setIgnoreTimeScale(false)
					 .setOnComplete(onComplete: () => { timeElapsedNormalTimeScale = Time.time; });
			...
			LeanTween.moveX(cubeIgnore, 5f, 1.5f).setIgnoreTimeScale(true)
					 .setOnComplete(onComplete: () => { timeElapsedIgnoreTimeScale = Time.time; });
			yield return new WaitForSeconds(1.5f);
			LeanTest.expect(Mathf.Abs(timeElapsedNormalTimeScale - timeElapsedIgnoreTimeScale) < 0.7f,
							"START IGNORE TIMING",
							"timeElapsedIgnoreTimeScale:" + timeElapsedIgnoreTimeScale +
							" timeElapsedNormalTimeScale:" + timeElapsedNormalTimeScale);

			//          yield return new WaitForSeconds(100f);
			Time.timeScale = 4f;
			int pauseCount = 0;
			LeanTween.value(gameObject, 0f, 1f, 1f).setOnUpdate(onUpdate: (float val) => { pauseCount++; }).pause();
			...
			LeanTween.moveLocal(cubeRound, roundCirc, 0.5f).setOnComplete(onComplete: () =>
																					  {
																						  LeanTest
																							 .expect(cubeRound.transform.position == onStartPos,
			...
			LeanTween.moveSplineLocal(cubeSpline, roundSpline, 0.5f).setOnComplete(onComplete: () =>
																							   {
																								   LeanTest.expect(Vector3.Distance(onStartPosSpline, cubeSpline.transform.position) <= 0.01f,
																												   ...
																												   "onStartPos:" + onStartPosSpline + " onEnd:" +
																												   cubeSpline.transform.position + " dist:" +
			...
			LTSeq seq = LeanTween.sequence().append(LeanTween.moveX(cubeSeq, 100f, 0.2f));
			...
			LeanTween.move(cubeBounds, new Vector3(10, 10, 10), 0.1f).setOnUpdate(onUpdate: (float val) =>
																							{
																							...
																							}).setLoopPingPong().setRepeat(8).setOnComplete(onComplete: () =>
			{
				LeanTest.expect(didPassBounds,
								"OUT OF BOUNDS",
								"pos x:" + failPoint.x + " y:" +
								failPoint.y + " z:" +
								failPoint.z);
			...
			for (int i = 0; i < groupTweens.Length; i++)
			{
				...
				groupTweens[i] = LeanTween.move(groupGOs[i], finalPos, 3f).setOnStart(onStart: () => { setOnStartNum++; })
										  .setOnUpdate(onUpdate: newPosition =>
																 {
																 ...
																 }).setOnCompleteParam(finalDict).setOnComplete(onComplete: param =>
																															{
																																...
																																if (neededPos.ToString() ==
																																	tweenedGo.transform.position.ToString())
																																	setPosNum++;
																																if (hasGroupTweensCheckStarted == false)
																																{
																																	hasGroupTweensCheckStarted = true;
																																	LeanTween.delayedCall(gameObject, 0.1f, callback: () =>
																																	{
																																		LeanTest
																																		   .expect(setOnStartNum == groupTweens.Length,
																																				"SETONSTART CALLS",
																																				"expected:" +
																																				groupTweens.Length +
																																				" was:" + setOnStartNum);
																																		LeanTest
																																		   .expect(groupTweensCnt == groupTweens.Length,
																																				"GROUP FINISH",
																																				"expected " +
																																				groupTweens.Length +
																																				" tweens but got " +
																																				groupTweensCnt);
																																		LeanTest
																																		   .expect(setPosNum == groupTweens.Length,
			...
			lt4 = LeanTween.moveX(cube4, 5.0f, 1.1f).setOnComplete(onComplete: () =>
																			   {
																				   LeanTest
																					  .expect(cube4 != null && previousXlt4 != cube4.transform.position.x,
																							  "RESUME OUT OF ORDER",
																							  "cube4:" + cube4 +
																							  " previousXlt4:" + previousXlt4 +
			...
			LeanTween.moveX(cube1, -5f, tweenTime).setOnUpdate(onUpdate: (float val) => { onUpdateWasCalled = true; })
					 .setOnComplete(onComplete: () =>
												{
													float end = Time.realtimeSinceStartup;
													float diff = end - start;
													LeanTest.expect(Mathf.Abs(expectedTime - diff) < 0.06f,
																	"SCALED TIMING DIFFERENCE",
																	"expected to complete in roughly " + expectedTime +
																	" but completed in " + diff);
													LeanTest.expect(Mathf.Approximately(cube1.transform.position.x, -5f),
																	"SCALED ENDING POSITION",
																	"expected to end at -5f, but it ended at " +
			...
			LeanTween.value(gameObject, new Vector3(1f, 1f, 1f), new Vector3(10f, 10f, 10f), 1f)
					 .setOnUpdate(onUpdate: val => { didGetCorrectOnUpdate = val.x >= 1f && val.y >= 1f && val.z >= 1f; })
					 .setOnComplete(onComplete: () => { LeanTest.expect(didGetCorrectOnUpdate, "VECTOR3 CALLBACK CALLED"); });
		...
		private IEnumerator lotsOfCancels()
		{
			...
			cubePaused.LeanMoveX(10f, 1f).setOnComplete(onComplete: () => { pauseTweenDidFinish = true; });
		...
		private IEnumerator pauseTimeNow()
		{
			yield return new WaitForSeconds(0.5f);
			Time.timeScale = 0;
			LeanTween.delayedCall(0.5f, callback: () => { Time.timeScale = 1f; }).setUseEstimatedTime(true);
			LeanTween.delayedCall(1.5f, callback: () => { LeanTest.expect(pauseTweenDidFinish, "PAUSE BY TIMESCALE FINISHES"); })
