...
public class GeneralBasics2d : MonoBehaviour
{
	...
	private void Start()
	{
		...
		LeanTween.moveX(avatarScale, avatarScale.transform.position.x + 1f, 5f)
				 .setEase(LeanTweenType
							 .easeOutBounce); // Simultaneously target many different tweens on the same object 
	...
	private GameObject createSpriteDude(string name, Vector3 pos, bool hasParticles = true)
	{
		...
		if (hasParticles)
		{
			GameObject particles = Instantiate(prefabParticles, Vector3.zero, prefabParticles.transform.rotation);
		...
		LeanTween.delayedCall(gameObject, 14f, callback: () =>
														 {
															 for (int i = 0; i < 10; i++)
															 {
																 ...
																 rotator.transform.position =
																	 new Vector3(2.71208f, 7.100643f, -12.37754f);

																 // Instantiate Avatar
																 GameObject dude =
																	 createSpriteDude("dude" + i,
																					  new Vector3(-2.51208f, 7.100643f,
																								  -14.37754f),
																 ...
																 // Push into the wheel
																 LeanTween.moveLocalZ(dude, -2f, 0.3f).setDelay(1.2f + i * 0.4f)
																 ...
																 LeanTween.moveLocalY(dude, 1.17f, 1.2f).setDelay(5f + i * 0.2f)
																		  .setLoopPingPong(1)
