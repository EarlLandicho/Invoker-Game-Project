...
public class TestingPunch : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Input.GetKeyDown(KeyCode.Q))
		{
			...
			LeanTween.moveLocalX(gameObject, 5, 1).setOnComplete(onComplete: () => { Debug.Log("on complete move local X"); })
			...
			LeanTween.value(lt.gameObject, lt.intensity, 0.0f, 1.5f)
					 ...
					 .setLoopPingPong()
					 .setRepeat(-1)
					 .setOnUpdate(onUpdate: val => { lt.intensity = val; });
		}
		...
		if (Input.GetKeyDown(KeyCode.S))
		{
			...
			LeanTween.rotateAround(gameObject, Vector3.forward, -360f, 0.3f).setOnComplete(onComplete: () =>
																									   {
																										   LeanTween.rotateAround(gameObject, Vector3.forward, -360f, 0.4f)
																													.setOnComplete(onComplete: () =>
																																   {
																																	   LeanTween.scale(gameObject, new Vector3(1f, 1f, 1f), 0.1f);
																																	   LeanTween.value(gameObject, callOnUpdate: v => { }, 0, 1,
																																		   0.3f).setDelay(1);
																																   });
																									   });
		}
		...
		if (Input.GetKeyDown(KeyCode.T))
		{
			...
			descr = LeanTween.move(gameObject, pts, 15f).setOrientToPath(true).setDirection(1f)
							 .setOnComplete(onComplete: () => { Debug.Log("move path finished"); });
		}
		...
		{
#if !UNITY_FLASH
			LeanTween.value(gameObject, callOnUpdate: val =>
													  {
														  // Debug.Log("tweening vec2 val:"+val);
														  transform.position =
															  new Vector3(val.x, transform.position.y, transform.position.z);
													  }, new Vector2(0f, 0f), new Vector2(5f, 100f), 1f)
	...
	private static void tweenStatically(GameObject gameObject)
	{
		...
		LeanTween.value(gameObject, callOnUpdate: val => { Debug.Log("tweening val:" + val); }, 0f, 1f, 1f);
	}
