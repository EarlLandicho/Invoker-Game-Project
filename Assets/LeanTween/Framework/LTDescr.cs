...
public class LTDescr
{
	...
	public void reset()
	{
		...
		hasUpdateCallback = useEstimatedTime =
								useFrames = hasInitiliazed =
												onCompleteOnRepeat =
													destroyOnComplete =
														onCompleteOnStart = useManualTime = hasExtraOnCompletes = false;
	...
	public LTDescr setMoveLocalX()
	{
		...
		easeInternal = () =>
					   {
						   trans.localPosition =
							   new Vector3(easeMethod().x, trans.localPosition.y, trans.localPosition.z);
					   };
		return this;
	}
	...
	public LTDescr setMoveLocalY()
	{
		...
		easeInternal = () =>
					   {
						   trans.localPosition =
							   new Vector3(trans.localPosition.x, easeMethod().x, trans.localPosition.z);
					   };
		return this;
	}
	...
	public LTDescr setMoveLocalZ()
	{
		...
		easeInternal = () =>
					   {
						   trans.localPosition =
							   new Vector3(trans.localPosition.x, trans.localPosition.y, easeMethod().x);
					   };
		return this;
	}
		...
		easeInternal = () =>
					   {
						   ...
						   val = newVect.x;
						   var origPos = trans.localPosition;
						   trans.RotateAround(trans.TransformPoint(_optional.point),
											  trans.TransformDirection(_optional.axis), -_optional.lastVal);
						   var diff = origPos - trans.localPosition;
						   trans.localPosition =
							   origPos -
							   diff; // Subtract the amount the object has been shifted over by the rotate, to get it back to it's orginal position
 ...
 = child.gameObject.renderer.material.color; this.fromInternal.x = col.a; break; }}}
			...
			break;
				...
				alphaRecursive(this.trans, val, this.useRecursion);
											  ...
											  {
												  spriteRen.color =
													  new Color(spriteRen.color.r, spriteRen.color.g, spriteRen.color.b,
																val);
		...
		easeInternal = () =>
			...
			alphaRecursive(this.trans, val, this.useRecursion);
						   ...
						   {
							   spriteRen.color =
								   new Color(spriteRen.color.r, spriteRen.color.g, spriteRen.color.b, val);
							   alphaRecursiveSprite(trans, val);
						   }
						   else
						   {
		...
		initInternal = () =>
					   {
						   ...
						   to = new Vector3(LeanTween.closestRot(fromInternal.x, toInternal.x),
											LeanTween.closestRot(from.y, to.y), LeanTween.closestRot(from.z, to.z));
					   };
		easeInternal = () =>
					   {
						   newVect = easeMethod();
		...
		easeInternal = () =>
					   {
						   ...
						   _optional.ltRect.rect =
							   new Rect(v.x, v.y, _optional.ltRect.rect.width, _optional.ltRect.rect.height);
					   };
		return this;
	}
		...
		easeInternal = () =>
					   {
						   ...
						   var rect = rectTransform;
						   var origPos = rect.localPosition;
						   rect.RotateAround(rect.TransformPoint(_optional.point),
											 rect.TransformDirection(_optional.axis), -val);
						   var diff = origPos - rect.localPosition;
						   rect.localPosition =
							   origPos -
							   diff; // Subtract the amount the object has been shifted over by the rotate, to get it back to it's orginal position
